...
local json   = require("json")  -- v2.1: para arg_str serializado nos hooks
local tools  = {}
tools.registry = {}
...
-- register: schema (opcional) habilita native tool calling para essa ferramenta
...
require("tools.web_fetch").register(tools, helpers) -- v3.1: web_fetch nativo
...
-- ── Dispatcher legado: call("name|raw_arg") ──────────────────────────────────
...
-- ── Dispatcher nativo: call_structured(name, args_table) ─────────────────────
function tools.call_structured(name, args)
  local tool = tools.registry[name]
  if not tool then return "❌ Ferramenta '" .. name .. "' não existe." end

  -- Fix 4: valida argumentos obrigatórios antes de executar
  -- Espelha filter_tc() do request_stream como segunda camada de proteção.
  ...
  local hooks_engine = require("agent.hooks.engine")
  -- Fix 1: arg_str cobrindo TODOS os campos (expression, name, session_id, etc.)
  -- Antes só cobria command/path/query/arg — hooks ficavam cegos para metade das tools.
  local arg_str
  if type(args) == "table" then
    ...
    if primary and type(primary) == "string" then
      arg_str = primary  -- campo principal → backward compat com hooks de bash patterns
    else
...
-- ── Schema para API: Fix 3 — ordem estável e determinística ──────────────────
-- pairs() não garante ordem; modelos são sensíveis à posição das tools na lista.
function tools.get_schema()
  local names = {}
  for name, data in pairs(tools.registry) do
    if data.schema then names[#names + 1] = name end
  end
  table.sort(names)  -- v2.1: ordem canônica alfabética a cada requisição
  local schema = {}
...
-- ── Legado: texto de tools para system prompt ─────────────────────────────────
