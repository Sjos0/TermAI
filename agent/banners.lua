...
function M.flush(tokens, mf_cfg, preview)
  local tw     = core.tw()
  local linha  = string.rep("━", tw)
  local titulo = "MEMORY FLUSH AUTOMÁTICO"
  local pad    = math.max(0, math.floor((tw - core.wlen(titulo)) / 2))
  local estado = mf.estado(tokens, mf_cfg)
  io.write("\n\27[38;5;220m" .. linha .. "\27[0m\n")
  io.write("\27[1m\27[38;5;220m" .. string.rep(" ", pad) .. titulo .. "\27[0m\n")
...
function M.compactacao(tokens, mf_cfg)
  local linha   = string.rep("━", core.tw())
